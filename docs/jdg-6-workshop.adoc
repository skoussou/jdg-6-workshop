...
* set the maven with a profile for the unzipped repository (eg file:../example-settings.xml[example-settings.xml])
 
We start with a simple JDG Embedded Setup Labe (file:../JdgLibraryWorkshopSetup1[JdgLibraryWorkshopSetup1]). Here we simply create a cache based on embedded JDG libraries and a default non-clustered configuration. Execute the application by running the `Main class. Observe the output. Why is the thread not exiting? Can you fix it?
----
