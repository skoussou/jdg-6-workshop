...
Lab 1 - JDG Cache in Libary (Embedded Mode)

We start with a simple JDG Embedded Setup Labe (JdgLibraryWorkshopSetup1). Here we simply create a cache based on default non-clustered configuration. Execute the application by running the `Main class. Observe the output. Why is the thread not exiting? Can you fix it?
----
