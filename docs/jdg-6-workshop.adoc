...
link:https://docs.jboss.org/infinispan/7.2/apidocs/[Cache]:: 
Is the central interface of Infinispan. It is based on the following hierarchy (ConcurrentMap is not shown) of interfaces and as with _CacheManger_ there are 2 types of Cache Interfaces (_Remote_, _Embedded_) which extend the _BasicCache_
...
* set the maven with a profile for the unzipped repository (eg ../example-settings.xml)
 
We start with a simple JDG Embedded Setup Labe ([../JdgLibraryWorkshopSetup1]JdgLibraryWorkshopSetup1). Here we simply create a cache based on embedded JDG libraries and a default non-clustered configuration. Execute the application by running the `Main class. Observe the output. Why is the thread not exiting? Can you fix it?
----
