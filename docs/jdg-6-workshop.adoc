...
We start with a simple JDG Embedded Setup Labe (**JdgLibraryWorkshopSetup1**). Here we simply create a cache based on embedded JDG libraries and a default non-clustered configuration. Execute the application by running the `Main class. Observe the output. Why is the thread not exiting? Can you fix it?
----
